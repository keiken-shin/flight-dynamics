...
**One scene, configured per chapter.** `src/sim/tasks.js` is pure data — no Three, no DOM — declaring for each chapter its brief, which controls to fit, which of the four arrows to draw, its four instrument rows, an optional `track` that watches for the thing being taught, and a latching goal. All twelve chapters have one. Drawing four arrows on a plate about drag would be three distractions, so lesson 4 draws two; a rudder pedal on that plate would be a thing to fiddle with rather than learn from, so it gets throttle and elevator only.

**A chapter appears there only if the model can demonstrate its idea honestly.** `MISSING` is empty now, but it stays in the file, because the rule it encodes is the one this world may never break: miming a Dutch roll the model is not computing would be worse than having no sandbox at all.
...
- **Don't** put two figure references back to back. The second opens a section with no prose in it, so its plate has no range to stick within and flashes past on a narrow screen. Chapter 11 does this and is the one place it shows.
...
- **Don't** leave a reader inside a chapter with no way out. The wordmark links home but reads as a title, so the bar carries an explicit `Index` control and the footer's middle slot carries `All 12 chapters`.
